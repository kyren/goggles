use goggles::{AnyLocalCloneComponentSet, AnyLocalComponentSet, Component, LocalWorld, VecStorage};
...
fn test_any_components() {
    ...
    let mut components_prefab = AnyLocalCloneComponentSet::new();
    components_prefab.insert::<CA>(CA(1));
    components_prefab.insert::<CB>(CB(2));

    let mut components = AnyLocalComponentSet::new();
